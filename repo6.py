...
def is_within_base_url(url, base_url):
    """Ensure the URL remains within the base URL scope."""
...
def find_latest_folder(base_url):
    ...
    for link in links:
        ...
        if href and href.endswith('/'):  # Look for folder links
            ...
            if is_within_base_url(full_url, base_url):  # Ensure it's within the base scope
                folders.append(href)
...
def find_specific_file(folder_url, partial_file_name, base_url):
    """
    ...
    for link in links:
        ...
        if href.endswith('/') and is_within_base_url(full_url, base_url):
            print(f"Checking subdirectory: {full_url}")
            file_url = find_specific_file(full_url, partial_file_name, base_url)
...
def main():
    ...
    try:
        ...
        file_url = find_specific_file(latest_folder_url, partial_file_name, base_url)
