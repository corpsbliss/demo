...
def find_latest_folder(base_url):
    """Find the latest folder from the webserver directory listing."""
    ...
    # Sort by folder names (assumes folders are named with timestamps or in sortable order)
    latest_folder = sorted(folders)[-1]
    print(f"Latest folder: {latest_folder}")
    return urljoin(base_url, latest_folder)

def find_specific_file(folder_url, partial_file_name):
    """Search for a file with a partial match in the folder and its subdirectories."""
    ...
    # Check for matching files or subdirectories
    for link in links:
        href = link.get('href')
        if not href:
            continue
...
def main():
    base_url = "http://your-webserver-url/"  # Replace with your webserver URL
