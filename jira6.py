...
# Function to trigger Jenkins job and get queue item URL
def trigger_jenkins_job(parameters):
    queue_item_number = server.build_job(JENKINS_JOB_NAME, parameters)
    print(f"Triggered Jenkins job: {JENKINS_JOB_NAME}, Queue item number: {queue_item_number}")
    return queue_item_number

# Function to get build number from queue item
def get_build_number_from_queue(queue_item_number):
    while True:
        queue_info = server.get_queue_item(queue_item_number)
        if 'executable' in queue_info:
            build_number = queue_info['executable']['number']
            print(f"Build number: {build_number} assigned to the queue item.")
            return build_number
        print(f"Waiting for build number for queue item {queue_item_number}...")
        time.sleep(5)
